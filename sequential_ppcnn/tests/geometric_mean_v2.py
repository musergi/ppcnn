    ...
    for layer in range(len(deltas[0])):
        ...
        for node_deltas in deltas:
            ...
            if gmean_layer_deltas is None:
                gmean_layer_deltas = node_deltas[layer]
            else:
                gmean_layer_deltas = gmean_layer_deltas * node_deltas[layer]
...
deltas_node3 = [
    np.array([[8, 2, 1], [3, 2, 1]]), # Layer 1
    np.array([[5, 2, 2], [3, 7, 2]])  # Layer 2
]
deltas = [deltas_node1, deltas_node2, deltas_node3]

geometric_mean_delta = np.power(np.prod(deltas, axis=0), 1/len(deltas))
